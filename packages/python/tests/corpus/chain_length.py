...
result = df.select("a").filter(F.col("a") > F.lit(0)).groupBy("a").agg(F.sum("a")).orderBy("a").limit(10)  # expect: SDK009
...
at_threshold = df.select("a").filter(F.col("a") > F.lit(0)).groupBy("a").agg(F.sum("a")).orderBy("a")  # expect: none
